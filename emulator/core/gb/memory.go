...
func (mem *Memory) LoadCart(loc string) (bool, error) {
	rom, err := os.ReadFile(loc)
	if err != nil {
		return false, err
	}
	return mem.LoadCartBytes(rom)
}

// LoadCartBytes loads a cart rom from a byte slice (used for tests and
// batch/headless runners). Carts without a backing file never write saves.
func (mem *Memory) LoadCartBytes(rom []byte) (bool, error) {
	mem.Cart = cart.NewCart(rom, "")
